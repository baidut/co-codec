...
**
** $Id: anssequentialscan.cpp,v 1.33 2012-09-23 14:10:12 thor Exp $
**
*/
...
/// ANSSequentialScan::Classify
// Find the DC context class depending on the previous DC and
// the values of L and U given in the conditioner.
struct ANSSequentialScan::QMContextSet::DCContextZeroSet &ANSSequentialScan::QMContextSet::Classify(LONG diff,UBYTE l,UBYTE u)
{
...
void ANSSequentialScan::WriteFrameType(class ByteStream *io)
{
  ...
  if (m_ucScanStart > 0 || m_ucScanStop < 63 || m_ucLowBit > hidden) {
    ...
    if (m_bDifferential)
      io->PutWord(JPGCODE_SOF_ANS_DIF_SEQ); // ANS differential sequential
    else
      io->PutWord(JPGCODE_SOF_ANS_SEQ); // ANS sequential
  }
}
///
